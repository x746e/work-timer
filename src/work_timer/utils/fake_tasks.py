...
def fake_tasks_from_tree(tree: 'Tree') -> list[FakeTask]:
    ...
    from work_timer.ui.task_list import _PRIO_TO_STYLE  # pylint: disable=import-outside-toplevel

    _STYLE_TO_PRIO = dict(map(reversed, _PRIO_TO_STYLE.items()))  # type: ignore  # pylint: disable=invalid-name

    def node_to_task(node):
        return FakeTask(
                node.data.title,
                status=node.data.status,
                priority=_STYLE_TO_PRIO[node.label.style],  # type: ignore
                kids=get_kids(node))
